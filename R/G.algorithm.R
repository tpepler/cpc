G.algorithm <- function(T.mat, nvec, method)
{
  ...
  if(method == 'LS'){ # Least Squares estimation
    while(abs(Q[2, 1] - Q.nuut[2, 1]) > 1e-07){
      ...
      for(i in 1:k){
        U <- U + nvec[i] * (delta[i, 1] - delta[i, 2]) * T.mat[, , i]
      }
      Q.nuut <- eigvec(U)
    }
  }
  else{ # else, assume the default: Maximum Likelihood estimation
    while(abs(Q[2, 1] - Q.nuut[2, 1]) > 1e-07){
      Q <- Q.nuut
      for(i in 1:k){
        for(j in 1:2){
