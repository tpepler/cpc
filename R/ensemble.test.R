ensemble.test <- function(origdata, standardize = FALSE)
{
  ...
  commonvec.order <- findcpc(covmats, B = B, plotting = FALSE)$commonvec.order
  ...
  resultmat <- rbind("Common eigenvector" = commonvec.order,
                 "Flury AIC" = fluryAIC.ind, "BVD" = BVD.ind, BCR = BCR.ind,
  ...
  return(list(Results = resultmat, commonvecs = commonvecnums, commonvecmat = B[, commonvecnums, drop = FALSE])) # Row 1: order of common eigenvectors in B; Row 2-5: results from AIC, BVD, BCR and RVC tests (1 = eigenvector common); Row 6: ensemble test common eigenvector indicator (1 = eigenvector common)
}
