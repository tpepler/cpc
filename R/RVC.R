RVC <- function(covmats, reps = 100000)
{
  ...
  for (r in 1:reps) {
    ...
    randvec1 <- randvec1 / c(sqrt(randvec1 %*% randvec1))
    randvec2 <- runif(n = p, min = -1, max = 1)
    randvec2 <- randvec2 / c(sqrt(randvec2 %*% randvec2))
    rand.dotproducts[r] <- abs(randvec1 %*% randvec2)
  }
  
  commonvecnums <-
    findcpc(covmats, plotting = FALSE)$all.correlations[1:p, ]
  for (i in 1:k) {
    j <- 2
    while (j <= p) {
  ...
  orig.dotproducts <-
    abs(diag(t(E[, , 1][, commonvec.order[, 1]]) %*% (E[, , 2][, commonvec.order[, 2]])))
  
  pvals <- rep(NA, times = p)
  for (j in 1:p) {
    pvals[j] <-
      length(rand.dotproducts[rand.dotproducts >= orig.dotproducts[j]]) / reps
  }
  
  return(data.frame(
    commonvec.order,
    vec.correlations = orig.dotproducts,
    p.values = pvals
  ))
}
