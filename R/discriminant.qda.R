discriminant.qda<-function(origdata,group,method=c("unbiased","pooled","cpc","fullcpccrossvalid"),B=NULL,standardize=F)
{
  ...
  if(method[1]=="fullcpccrossvalid"){
    ...
    alpha1<-alpha.crossvalid2(group1data,B=B,reps=100)
    alpha2<-alpha.crossvalid2(group2data,B=B,reps=100)
  ...
  for(i in 1:(n.test1+n.test2)){
    newobs<-as.matrix(testdata[i,,drop=F])
  ...
  return(list(cbind(testdata,group=group.test,group.predict=group.predict),misclassrate))
}
