biplot.measures<-function(datalist,projectmat,rdim)
{
  ...
  axis.predictivities<-matrix(axis.predictivities,ncol=p,byrow=T)
  ...
  mspe<-rep(NA,times=p)
  for(j in 1:p){
    mspe[j]<-(t(onevec)%*%abs(X[,j]-X%*%projectmat[,1:rdim]%*%t(projectmat[j,1:rdim,drop=F])))/(n*Xsd[j])
  }
  mspe.mean<-mean(mspe)
  
  return(list(overall.quality=overall.quality,within.quality=within.quality,within.quality.mean=within.quality.mean,between.quality=between.quality,adequacies=adequacies,adequacies.median=adequacies.median,axis.predictivities=axis.predictivities,axis.predictivities.mean=axis.predictivities.mean,sample.predictivities=sample.predictivities,sample.predictivities.mean=sample.predictivities.mean,mspe=mspe,mspe.mean=mspe.mean))
}
